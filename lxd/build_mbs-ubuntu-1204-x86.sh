...
CONTAINER=mbs-ubuntu-1204-x86-tmp
IMAGE=mbs-ubuntu-1204-x86
...
lxc launch ubuntu:12.04/i386 ${CONTAINER}
...
lxc exec ${CONTAINER} -- bash -c 'echo "deb http://download.opensuse.org/repositories/home:/doc_bacardi/xUbuntu_12.04/ /" > /etc/apt/sources.list.d/build_opensuse_org_home_docbacardi.list'
lxc exec ${CONTAINER} -- bash -c 'curl --location --silent http://download.opensuse.org/repositories/home:doc_bacardi/xUbuntu_12.04/Release.key | apt-key add -'
lxc exec ${CONTAINER} -- apt-get update
...
lxc publish ${CONTAINER} --alias=${IMAGE} description="This image contains the build environment for the MBS system on Ubuntu 12.04 32 bit."
